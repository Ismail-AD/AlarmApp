...
fun ArrangeAlphabetsMHScreen(
    ...
    var correctList = remember { mutableStateOf(('A'..'Z').shuffled().take(elementsCountToPick)) }
...
//                    }
    ...
    LaunchedEffect(
        ...
        if (correctList.value.isEmpty() && countdown > 0) {
            correctList.value = ('A'..'Z').shuffled().take(elementsCountToPick)
        ...
        if(countdown > 0) {
            ...
            Box(
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Log.d("CHKSW", "$showWrong current value ")
                    AnimatedVisibility(
                        visible = showWrong,
                ...
                Box(contentAlignment = Alignment.Center, modifier = Modifier.fillMaxWidth()) {
                    if (countdown > 0) {
                        ...
                        LazyVerticalGrid(
                            ...
                            items(shuffledList.value, { it }) { item ->
                                ReorderableItem(
                                    state,
                                    key = item,
